...
;;
;; Shared shape with the other cloud-itonami record repositories. The floor is
;; read from the catalogue (`:catalog/min-citations`) rather than from a flag
;; default, so a catalogue that loses rows cannot pass on a stale `--min`.
;;
